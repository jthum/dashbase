...
function renderManualComponentFile(contract: ComponentContract) {
  ...
  return [
    ...
    `/* Generated by scripts/targets/react/generate.ts for ${contract.name}. */`,
...
function renderAutoEntryFile(contract: ComponentContract, outputFile: string) {
  ...
  if (getGeneratedAdapterMode(contract, "react") === "browser-shim") {
...
function renderManifestFile(contracts: ComponentContract[], patternManifests: Array<{ slug: string; assets: DashbaseAssetManifest }>) {
  ...
  lines.push("");
  lines.push("export const dashbaseReactPatternManifest = {");
...
function renderPackageReadme(
  ...
  lines.push("");
  lines.push("These files are generated adapter outputs with pilot contract coverage, not yet the final published package format.");
  lines.push("");
  return lines.join("\n");
}
...
function renderComponentReadme(contract: ComponentContract, examples: ResolvedDocExample[]) {
  ...
  const adapterMode = getGeneratedAdapterMode(contract, "react");
  const lines = [
    `# ${contract.name}`,
    "",
    contract.summary,
  ...
  if (isClientHostedComponent(contract, "react")) {
    lines.push("- In SSR frameworks, mount this component from a client boundary or after client hydration.");
  }

  if (contract.behavior.domOwnership === "shim-mutates-live-state") {
    lines.push("- The shim mutates live DOM state. If a framework starts fighting those mutations, prefer a controller-backed or native override.");
  }

  for (const note of getAdapterNotes(contract, "react")) {
    lines.push(`- ${note}`);
  }
...
function renderPatternManualFile(
  ...
  return [
    `/* Generated by scripts/targets/react/generate.ts for ${entry.contract.title}. */`,
    'import React from "react";',
...
async function generateReact() {
  ...
  console.log(`Generated package-style React target for ${contracts.length} component contracts and ${patternContracts.length} patterns in ${join("generated", "react")}.`);
}

generateReact().catch((error) => {
  console.error(error);
  process.exit(1);
});
