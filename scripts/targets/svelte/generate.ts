...
function renderComponentAutoIndex(contract: ComponentContract, targets: GeneratorTarget[]) {
  ...
  if (getGeneratedAdapterMode(contract, "svelte") === "browser-shim") {
...
function renderComponentReadme(contract: ComponentContract, examples: ResolvedDocExample[]) {
  const exportNames = getGeneratorTargets(contract).map((target) => target.exportName).join(", ");
  const adapterMode = getGeneratedAdapterMode(contract, "svelte");
  const lines = [
    `# ${contract.name}`,
    "",
    contract.summary,
  ...
  if (isClientHostedComponent(contract, "svelte")) {
  ...
  for (const note of getAdapterNotes(contract, "svelte")) {
    lines.push(`- ${note}`);
  }
