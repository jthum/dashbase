...
```html
<carousel role="region" aria-roledescription="carousel" aria-label="Launch feature tour">
  <carousel-track>
    <carousel-slide role="group" aria-roledescription="slide" aria-label="1 of 2">
      <article>Track signups and activation in the same view.</article>
    </carousel-slide>
    <carousel-slide role="group" aria-roledescription="slide" aria-label="2 of 2">
  ...
  <carousel-controls>
    ...
    <carousel-dots aria-label="Feature tour slides">
...
Use `<carousel class="vertical">` when the track should snap vertically instead of horizontally. Vertical slides keep their natural content height by default, and you can still override `--carousel-slide-block-size` when you want full-height panels. Dashbase expects slide semantics and dot buttons to be authored in the markup rather than generated by JavaScript. When multiple items are visible at once, the helper treats pagination as reachable snap positions and hides any extra authored dots that do not correspond to a distinct snap point.
